...
PWA: `manifest.json` + `sw.js`. **Versioning: bump `GAME_VERSION` in `js/version.js` ONCE per change batch** (current: v66) — it is the single source of truth: `sw.js` builds its cache name from it via `importScripts('js/version.js')`, and the main menu shows it in the bottom `#verBadge` so a deployed push is verifiable at a glance in the live game. Do NOT hardcode a version anywhere else.
...
- [x] BUILDING REDESIGN phase 1 (v63): _bRedesign painter map overrides bSpr for the core five with distinct silhouettes — command (two-tier HQ, comms spire+dish, helipad), power (twin glowing cooling stacks, pipe manifold), supply (corrugated warehouse, loading dock+pallets), barracks (pitched roof, sandbag entrance), factory (sawtooth skylight roof, hazard vehicle bay, smokestack+crane). Construction kit layers on top. v64 adds market (striped bazaar canopy, goods), tech (glowing reactor ring + server racks), radar (lattice dish over sweep rings), repairbay (open service pit, chassis, gantry, tool wall). v65 completes ALL buildings: turret (octagon bunker+bolted ring), samsite (angled quad launcher, red-tip missiles), silo (bold split doors, dashed warning ring, gantry), airfield (runway stripes, pad circles, control tower+windsock), watchtower (X-braced legs, searchlight). REDESIGN COMPLETE — tunnel kept as-is (pit). v66 adds _bFlourish faction dressing on every building: vanguard solar panels+whip antenna, crimson hanging star banners, scorpion tarps/scrap/cables, northwind frost crust+braziers. in the same visual language, verify via test/shot_build.sh screenshots.

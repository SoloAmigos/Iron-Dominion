...
function aiTick(tick){
  ...
  const sites=builds.filter(b=>b.team===1&&!b.dead&&!b.built);
  ...
  for(const b of builds){
    ...
    if(b.type==='factory'){
      ...
      if(fs.length&&Math.random()<.35)pick=fs[Math.floor(Math.random()*fs.length)].unit;
      const c=costOf('u',pick,1);
      if(money[1]>=c){money[1]-=c;b.queue.push({type:pick,p:0})}
    }else if(b.type==='barracks'){
      let pick=Math.random()<.45?'rocket':'ranger';
      const bs2=sigs.filter(g=>g.at==='barracks');
      if(bs2.length&&Math.random()<.4)pick=bs2[Math.floor(Math.random()*bs2.length)].unit;
      const c=costOf('u',pick,1);
  ...
  if(ai.waveT<=0){
    ...
    if(army.length>=5){
      ...
      for(const b of builds)if(!b.dead&&b.team===0){const d=dist2(b,ref);if(d<bd){bd=d;tgt=b}}
      if(tgt){

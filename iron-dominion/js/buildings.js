...
function placeBuilding(type,team,tx,ty,instant){
  ...
  for(const u of units)if(!u.dead&&u.x>tx*TILE-8&&u.x<(tx+t.w)*TILE+8&&u.y>ty*TILE-8&&u.y<(ty+t.h)*TILE+8){
    const f=freeNear(TT(u.x),TT(u.y));
    if(f){u.path=[{x:f.x*TILE+TILE/2,y:f.y*TILE+TILE/2}];u.wpi=0}
  }
  return b;
}
...
function completeBuilding(b){
  ...
  if(b.type==='supply')spawnUnit('truck',b.team,b.rally.x,b.rally.y);
  if(b.team===0){
...
function updateBuilding(b,dt){
  ...
  if(b.queue.length){
    ...
    if(it.p>=UT[it.type].bt){
      ...
      if(Math.hypot(b.rally.x-ex.x,b.rally.y-ex.y)>TILE)orderMove(nu,b.rally.x+rand(-14,14),b.rally.y+rand(-14,14),'move');
...
function recomputePower(){
  for(let t=0;t<2;t++){
    ...
    let p=0,u=0;
    for(const b of builds)if(!b.dead&&b.built&&b.team===t){
